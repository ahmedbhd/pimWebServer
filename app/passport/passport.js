...
module.exports = function(app, passport) {
    ...
    passport.use(new GoogleStrategy({
            clientID: '70745688119-jfrsp1in59dcqm2scq5dvt01n8nbvtp4.apps.googleusercontent.com', // Replace with your Google Developer App client ID
            clientSecret: 'QzRrucpSGP-o1NnPJfOhxq-t', // Replace with your Google Developer App client ID
            callbackURL: "http://localhost:8080/" // Replace with your Google Developer App callback URL
        },
        function(accessToken, refreshToken, profile, done) {
            User.findOne({ email: profile.emails[0].value }).select('username active password email').exec(function(err, user) {
                if (err) done(err);
